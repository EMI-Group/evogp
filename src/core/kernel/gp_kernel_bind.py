...
def gp_eval_(
    ...
    `prefixGPs` : `jax.Array(shape=(pop_size, max_len))`
        The prefix arrays of all GPs in the population, storing all functions (see [kernel.gpdefs.Function](src/kernel/utils.py)), constants and variable indices as floating point type and corresponding node types and subtree sizes
    ...
    Note that none of the array contents (except the multi-output indices) are checked for performance issues. Hence, make sure that the max stack size do not exceed [MAX_STACK](src/kernel/gpdefs.h).
    """
...
def gp_crossover_(
    ...
    Note that the `left_perms` are not checked and assumed to be within the range of [0, `pop_size`), while output GP(s) with invalid `right_perms` is/are direct copies of `gp[left_perms[i]]`. Besides, any crossover(s) that may cause size overflow is/are ignored as well.
    """
...
def gp_mutation_(
    ...
    `new_subtree_prefixGPs` : `jax.Array(shape=(pop_size, max_new_len))`
        The prefix arrays of all new subtree GPs in the population, storing all functions (see [kernel.gpdefs.Function](src/kernel/utils.py)), constants and variable indices as floating point type and corresponding node types and subtree sizes
...
def gp_generate_(
    ...
    """
    The (forward) function generating a population of GPs via given sizes and probabilities.
    ...
    `output_prob` : `float`
        The change that a function node in a generated GP tree is selected to be an output node. Not used when `output_len <= 1`.
