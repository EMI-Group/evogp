...
from src.utils.enum import FUNCS
from functools import partial
...
class LambdaPointMutation(Mutation):
    ...
    def __call__(self, key, trees, const, config) -> jax.Array:  # prefix_trees (in cuda)
        ...
        keys = jax.random.split(k3, pop_size * num_mutated_node)
        probs = jnp.tile(config["func_prob"], (pop_size * num_mutated_node, 1))
        ...
        random_var = jax.random.choice(k4, vars, shape=(pop_size * num_mutated_node,))
        mutated_node_value = jnp.where(mutated_node_type == 0, random_var, mutated_node_value)
        random_const = jax.random.choice(k5, consts, shape=(pop_size * num_mutated_node,))
