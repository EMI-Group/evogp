...
from evogp.algorithm import (HoistMutation,
    SinglePointMutation,
    LambdaPointMutation,
    InsertMutation,
    DeleteMutation)
...
def main():
    try:
        with open(f"ant_{sys.argv[1]}.csv", 'x') as file:
            file.write('gen')
    except:
        pass
    ...
    for i in range(200):
        ...
        update_csv(f"ant_{sys.argv[1]}.csv", i, np.max(fitnesses), np.min(fitnesses), np.mean(fitnesses), time.time() - start_time)
