...
def main():
    ...
    try:
        with open(f"halfcheetah_{sys.argv[1]}.csv", 'x') as file:
            file.write('gen')
    except Exception:
        pass
    ...
    for i in range(200):
        ...
        update_csv(f"halfcheetah_{sys.argv[1]}.csv", i, np.max(fitnesses), np.min(fitnesses), np.mean(fitnesses), time_per_gen, env_time)
