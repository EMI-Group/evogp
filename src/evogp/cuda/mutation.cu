...
__host__ __device__ inline void _gpTreeReplace(
    ...
    while (current < old_node_idx) {
        ...
        case NodeType::BFUNC:
            ...
            if (old_node_idx < rightTreeIndex) { // at left subtree
                                                 // do nothing
...
__global__ void treeGPMutationKernel(
    ...
    if (old_node_idx < 0 || old_node_idx >= old_size) { // invalid node index
    ...
    if (old_size + sizeDiff > maxGPLen) { // too large output size
        // (do nothing) copy old tree to output
        for (int i = 0; i < old_size; i++) {
            out_value[i] = old_value[i];
            out_type[i] = old_type[i];
...
__global__ void treeGPCrossoverKernel(
    ...
    if (right_idx[n] < 0 || right_idx[n] >= pop_size_ori) { // invalid right
    ...
    if (left_size + size_diff > maxGPLen) { // too large output size
        for (int i = 0; i < left_size; i++) {
            // directly copy left
            out_value[i] = left_value[i];
            out_type[i] = left_type[i];
