...
def main():
    try:
        with open(f"len100_{sys.argv[3]}.csv", 'x') as file:
            file.write('pop')
    except:
        pass
    ...
    update_csv(f"len100_{sys.argv[3]}.csv", alg.config['pop_size'], datapoint, -np.max(fitnesses), end_time - start_time)
    # start_time = time.time()
    # for i in range(20):
    #     state, fitnesses = jit_step(state)
    #     print(i)
